...
export default (editor, component, sectionAttrs) => {
  if (component.getAttributes()['data-app-name'] == 'faq') {
    const faqTraits = [
      {
        type: 'checkbox',
        name: 'showTitle',
    ...
    component.set({
      ...
      traits: [...faqTraits, ...sectionAttrs.traits]
    });
    ...
    function handleShowTitleChange(model, value) {
      const titleNodes = model.find('.faq_question');
    ...
    function handleShowShortDescriptionChange(model, value) {
      const shortDescriptionNodes = model.find('.faq_answer');
